...
面向 Linux 服务器的交互式脚本，用于管理 **DNAT 端口转发**，并在每条规则上可选配置 **按月回程流量限额**。默认版本标识为 **v1.1**。
...
| **新增转发** | 将 **本机监听端口（TCP + UDP）** 转发到指定 **目标 IP:端口**；支持备注；可选 **月流量上限**。 |
...
3. **新增端口转发** — 本机端口 → 目标 IP/端口；冲突检测（`ss`）；选择月上限（不限制 / 50G～800G）。
4. **删除端口转发**。
5. **一键清空所有转发**。
...
**定时任务**：`nft-forward-traffic-check.timer` 默认约 **每 5 分钟** 触发一次 `nft.sh --traffic-check`，用于更新用量并在超限时重写规则。
